...
```typescript
import { Axiom } from "axiom-js";
import * as errors from "axiom-js/models/errors";
...
async function run() {
    ...
    let result;
    try {
        result = await axiom.getAnnotation(id);
    } catch (err) {
        switch (true) {
...
```typescript
import { Axiom } from "axiom-js";
import { HTTPClient } from "axiom-js/lib/http";
...
const sdk = new Axiom({ httpClient });
```
